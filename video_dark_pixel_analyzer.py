...
class VideoDarkPixelAnalyzer:
    ...
    def save_time_differences(self, out_dir=None, remove_outliers=True):
        """
        Speichert die Periodendauer ('time_difference_to_previous') in einer Textdatei.
        Filtert Werte, die kleiner oder gleich 0,5s sind.
        ...
        try:
            ...
            # Sammle time_difference_to_previous Werte (> 0.5 Sekunden)
            time_diffs = []
...
def main():
    ...
    try:
        ...
        # NEU: Optional einen Analyse-Bereich für alles auswählen
        analysis_roi = None
        if use_last_settings and last_settings.get('analysis_roi'):
            analysis_roi = tuple(last_settings.get('analysis_roi'))
            print(f"Verwende gespeicherten Analyse-Bereich: {analysis_roi}")
        else:
            try:
                choice = input("\nMöchten Sie den Analyse-Bereich auf eine bestimmte Region beschränken? (j/n, Standard: n): ").strip().lower()
                if choice in ['j', 'ja', 'y', 'yes']:
                    analysis_roi = analyzer.select_analysis_region(first_frame)
                    print(f"Analyse-Bereich ausgewählt: {analysis_roi}")
            except EOFError:
                pass
        ...
        analyzer.save_time_differences()
